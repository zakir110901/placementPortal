...
def my_universities(request):
    x = company.objects.get(Email_ID = request.user.email)
    if request.method == 'GET':
        y = company_universities.objects.filter(company_name = x)
        a=[]
        for i in y:
            a.append(i.universities)
        a = set(a)
        b=[]
        p = University.objects.all()
        for i in p:
            b.append(i.College)
        b = set(b)
        z = b - a
        ...
        z = list(z)
        if(z):
            return render(request, 'superset/company_dashboard.html',{'mess_count':1 ,"count":7, 'a':a, 'z':z})
        else:
            return render(request, 'superset/company_dashboard.html',{'mess_count':0 ,"count":7, 'a':a})
    else:
        k = request.POST.getlist('uni')
        print(k)
        p_object = company.objects.get(Company_Name=x)
        u_objects = []
        for u in k:
            y = company_universities(company_name=p_object, universities=u)
            u_objects.append(y)
...
def uni_details(request, i):
    if request.method == 'GET':
        x = University.objects.get(College = i)
        return render(request, 'superset/company_dashboard.html',{"count":8, 'x':x})
    else:
        z = company.objects.get(Email_ID = request.user.email)
        y = company_universities.objects.filter(universities = i).filter(company_name = z)
        y.delete()
        return redirect("my_universities")